...
export const logo = `<svg viewBox="0 0 310 185" aria-label="rndyt 博客" role="img">${paths}<text x="165" y="174" text-anchor="middle" font-family="MiSans,sans-serif" font-size="16" font-weight="700" letter-spacing="22">RNDYT·LOG</text></svg>`;
...
export const brandHeading = `<h1>RNDYT</h1><div>THOUGHTS INTO ARCHIVES</div><p><span class="brand-analysis" role="img" aria-label="ANALYSIS">${[..."ANALYSIS"].map((letter, i) => `<span aria-hidden="true" style="left:${analysisPositions[i]}px">${letter}</span>`).join("")}</span> <b>OS</b></p>`;
