...
export function readerPage(record, all) {
  ...
  const content = record ? `<a href="/blog/">← 文章目录</a><header><div class="eyebrow">${escape(record.category)} / ${record.id}</div><h1>${escape(record.title)}</h1><p class="description">${escape(record.abstract)}</p><p class="meta">${escape(record.lead)} · ${escape(record.date)} · ${record.readingMinutes} 分钟阅读</p><div class="tags">${record.tags.map(tag => `<span>#${escape(tag)}</span>`).join(' ')}</div><a class="immersive" href="/?post=${record.slug}">在三维档案中打开 ↗</a></header>${record.preview ? '<aside class="notice">示例内容 · 尚未替换为真实文章或项目资料。</aside>' : ''}${record.headings.length ? `<nav class="toc" aria-label="文章目录">${record.headings.map(h => `<a href="#${h.id}">${h.text}</a>`).join('')}</nav>` : ''}<article class="prose">${record.bodyHtml}</article>${related.length ? `<section class="related"><h2>同类文章</h2>${related.map(post => `<a href="${post.source}">${escape(post.title)} ↗</a>`).join('')}</section>` : ''}` : '';
  return page(record.title, content, record.abstract);
}
export function indexPage(records, categories) {
  const sorted = [...records].sort((a,b) => b.date.localeCompare(a.date));
  return page('文章目录', `<header><div class="eyebrow">RNDYT / PERSONAL ARCHIVE</div><h1>文章目录</h1><p class="description">工程文章、AI 实践与项目记录。</p><a class="immersive" href="/">进入三维档案 ↗</a></header>${records.some(r => r.preview) ? '<aside class="notice">当前包含示例内容，不代表真实经历或成果。</aside>' : ''}${categories.map(category => `<section class="collection"><h2>${escape(category)}</h2>${sorted.filter(post => post.category === category).map(post => `<a class="post-row" href="${post.source}"><span class="meta">${post.date} / ${post.id}</span><h3>${escape(post.title)}</h3><p>${escape(post.abstract)}</p><span class="meta">${post.readingMinutes} 分钟阅读 ↗</span></a>`).join('')}</section>`).join('')}`);
}
function page(title, content, description = 'rndyt 的个人博客') {
  return `<!doctype html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="robots" content="noindex, nofollow"><meta name="description" content="${escape(description)}"><title>${escape(title)} · rndyt</title><link rel="icon" href="/favicon.svg"><link rel="stylesheet" href="/blog-reader.css"></head><body><main>${content}</main><footer>rndyt · 个人博客 <a href="/">三维档案</a> <a href="https://github.com/LBEILC/RhineLabUI">界面基于 RhineLabUI</a></footer></body></html>`;
}
